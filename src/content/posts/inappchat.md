---
title: "In-App Chat"
date: "2021-03-10"
tags: ["work"]
excerpt: "UX/UI design, web design, and motion graphics for a messaging SDK WebApp."
feature_image: "/posts/56/features.jpg"
roles: "UX/UI Design, Web Design, Motion Graphics"
tools: "Figma"
---

In-App Chat is a messaging SDK WebApp. I designed the full UX/UI for the messenger interface, created the marketing website and onboarding flow, and produced feature preview GIFs for promotional use.

**Roles:** UX/UI Design, Web Design, Motion Graphics

**Tools:** Figma

![](/posts/56/features.jpg)

![](/posts/56/main.png)

![](/posts/56/Screenshot-2021-03-09-201704.png)

![](/posts/56/main1.png)
