---
title: "Dutch Bros Coffee"
date: "2022-01-24"
tags: ["work"]
excerpt: "Design systems management, Sketch-to-Figma conversion, and UI support for Dutch Bros loyalty app at Hathway."
feature_image: "/posts/50/Typography.png"
roles: "Design Systems Management, UI Support, Graphic Design"
tools: "Figma, Sketch, Photoshop, Illustrator"
agency: "Hathway"
---

Dutch Bros is a coffee chain in the western USA which approached Hathway with the goal of improving their loyalty program. I helped bring this project from Sketch to Figma, converting all symbols into components, creating new global type and color styles, and ensuring proper architecture for each component. I created a new, clearer layout for the design system and continue to manage it as needed.

**Roles:** Design Systems Management, UI Support, Graphic Design

**Tools:** Figma, Sketch, Photoshop, Illustrator

**Agency:** Hathway

![](/posts/50/Typography.png)

![](/posts/50/Color-Palette.png)

![](/posts/50/Molecules.png)

![](/posts/50/Atoms.png)

![](/posts/50/Frame-229.png)

![](/posts/50/Frame-230.png)

![](/posts/50/Organisms.png)

![](/posts/50/slice3.png)

![](/posts/50/slice1.png)

![](/posts/50/Slide-16_9---1.png)

![](/posts/50/slice5.png)

![](/posts/50/slice2.png)

![](/posts/50/MAIN.png)
